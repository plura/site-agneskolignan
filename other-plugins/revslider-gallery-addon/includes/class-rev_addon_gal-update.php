...
class RevAddOnGalUpdate {
	private $plugin_url		 = 'https://codecanyon.net/item/slider-revolution-responsive-wordpress-plugin/2751380';
	private $remote_url_info = 'addons/revslider-gallery-addon/revslider-gallery-addon.php';
	private $plugin_slug	 = 'revslider-gallery-addon';
	private $plugin_path	 = 'revslider-gallery-addon/revslider-gallery-addon.php';
	private $version;
	private $plugins;
	private $option;
	...
	protected function _check_updates($force_check = false){
		...
		$last_check = get_option('rev_addon_gal-update-check');

		if($last_check == false){ //first time called
			$last_check = time();
			update_option('rev_addon_gal-update-check', $last_check);
		}
		...
		if(time() - $last_check > 60 * 60 * 24 * 30 || $force_check == true){
			...
			if(isset($data->basic)){
				update_option('rev_addon_gal-update-check', time());
				
				$this->data->checked = time();
				$this->data->basic = $data->basic;
				$this->data->full = $data->full;
				
				update_option('rev_addon_gal-latest-version', $data->full->version);
			}
		}
	...
	public function _retrieve_update_info(){
		...
		$rattr = array(
			...
			'version' => urlencode(REV_ADDON_GAL_VERSION)
		);
